...
pub fn run(
    ...
    match ext {
        "png" => {
            ...
            pastel_render::export::export_png(&mut surface, output).map_err(|e| e.to_string())?;
            println!("  Rendered {} -> {}", file.display(), output.display());
        }
        "svg" => {
            pastel_render::export::export_svg_file(&ir, output).map_err(|e| e.to_string())?;
...
fn render_single(
    ...
    output: &Path,
    ir: &pastel_lang::ir::IrDocument,
    nodes: &[pastel_lang::ir::node::IrNode],
    ext: &str,
) -> Result<(), Box<dyn std::error::Error>> {
    match ext {
        "png" => {
