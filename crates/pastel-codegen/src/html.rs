...
pub fn generate_html(ir: &IrDocument) -> String {
    ...
    let nodes = if ir.pages.is_empty() {
        &ir.nodes
    } else {
        &ir.pages[0].nodes
    };
    ...
    match &node.data {
        ...
        IrNodeData::Image(img) => {
            ...
            let fit = img
                .fit
                .as_ref()
                .map(|f| format!("object-fit:{};", fit_css(f)))
                .unwrap_or_default();
            out.push_str(&format!(
                "{pad}<img src=\"\" alt=\"{}\" style=\"{w}{h}{fit}\" />\n",
...
fn frame_style(f: &pastel_lang::ir::node::FrameData) -> String {
    ...
    if let Some(w) = &f.width {
        s.push(format!("width:{}", dim_val(w)));
    }
    if let Some(h) = &f.height {
        s.push(format!("height:{}", dim_val(h)));
    }
    ...
    if let Some(layout) = &f.layout {
        match layout.mode {
            LayoutMode::Horizontal => {
                s.push("display:flex".into());
            }
            LayoutMode::Vertical => {
                s.push("display:flex".into());
                s.push("flex-direction:column".into());
            }
            LayoutMode::Grid => {
                s.push("display:grid".into());
                if let Some(cols) = layout.columns {
                    s.push(format!("grid-template-columns:repeat({},1fr)", cols));
                }
            }
            LayoutMode::Stack => {
                s.push("display:grid".into());
        ...
        if let Some(gap) = layout.gap {
            s.push(format!("gap:{}px", gap));
        }
        if let Some(a) = &layout.align {
            s.push(format!("align-items:{}", align_css(a)));
        }
        if let Some(j) = &layout.justify {
            s.push(format!("justify-content:{}", justify_css(j)));
        }
    }
    if let Some(fill) = &f.visual.fill {
        s.push(fill_css(fill));
    }
    ...
    if let Some(r) = &f.visual.corner_radius {
        let [a, b, c, d] = r.0;
        s.push(format!("border-radius:{}px {}px {}px {}px", a, b, c, d));
    }
    ...
    if let Some(o) = f.visual.opacity {
        if o < 1.0 {
            s.push(format!("opacity:{}", o));
        }
    }
    s.join(";")
}
...
fn text_style(t: &pastel_lang::ir::node::TextData) -> String {
    ...
    if let Some(fs) = t.font_size {
        s.push(format!("font-size:{}px", fs));
    }
    if let Some(w) = &t.font_weight {
        s.push(format!("font-weight:{}", w.to_css_value()));
    }
    if let Some(f) = &t.font_family {
        s.push(format!("font-family:'{}'", f));
    }
    if let Some(c) = &t.color {
        s.push(format!("color:{}", c.to_hex()));
    }
    if let Some(a) = &t.text_align {
        s.push(format!("text-align:{:?}", a).to_lowercase());
    }
    if let Some(lh) = t.line_height {
        s.push(format!("line-height:{}px", lh));
    }
    if let Some(ls) = t.letter_spacing {
        s.push(format!("letter-spacing:{}px", ls));
    }
    s.join(";")
}
...
fn dim_val(d: &Dimension) -> String {
    match d {
        Dimension::Fixed(n) => format!("{}px", n),
        Dimension::Fill => "100%".into(),
        Dimension::Hug => "fit-content".into(),
    }
}

fn dim_css(d: Option<&Dimension>) -> String {
    d.map(|d| format!("width:{};", dim_val(d)))
        .unwrap_or_default()
}
