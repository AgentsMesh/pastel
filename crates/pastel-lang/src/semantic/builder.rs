    ...
    fn build_frame(&self, node: &NodeDecl) -> Result<FrameData, PastelError> {
            ...
            match attr.key.as_str() {
                ...
                "stroke-dash" => {
                    let dash = p
                        ...
                        .map_err(|e| e.with_span(attr.span))?;
                    if let Some(ref mut s) = f.visual.stroke {
                        s.dash = Some(dash);
                    }
                }
                "radius" => {
                    f.visual.corner_radius = Some(
                        p.resolve_corners(&attr.value)
                            .map_err(|e| e.with_span(attr.span))?,
                    )
                }
                "shadow" => {
                    f.visual.shadow = Some(
                        p.resolve_shadow(&attr.value)
                            .map_err(|e| e.with_span(attr.span))?,
                    )
                }
                "inner-shadow" => {
                    f.visual.inner_shadow = Some(
                        p.resolve_shadow(&attr.value)
                            .map_err(|e| e.with_span(attr.span))?,
                    )
                }
                "opacity" => {
                    f.visual.opacity = Some(
                ...
                "layout" => {
                    mode = Some(
                        p.resolve_layout_mode(&attr.value)
                            .map_err(|e| e.with_span(attr.span))?,
                    )
                }
                "gap" => {
                ...
                "align" => {
                    align = Some(
                        p.resolve_align(&attr.value)
                            .map_err(|e| e.with_span(attr.span))?,
                    )
                }
                "justify" => {
                    justify = Some(
                        p.resolve_justify(&attr.value)
                            .map_err(|e| e.with_span(attr.span))?,
                    )
                }
                "columns" => {
                    columns = Some(
                        p.resolve_u32(&attr.value)
                            .map_err(|e| e.with_span(attr.span))?,
                    )
                }
                "rows" => {
        ...
        if mode.is_some() || gap.is_some() || align.is_some() || justify.is_some() {
            f.layout = Some(Layout {
                mode: mode.unwrap_or(LayoutMode::Vertical),
                gap,
                align,
                justify,
                columns,
