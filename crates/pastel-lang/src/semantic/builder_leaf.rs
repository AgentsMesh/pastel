...
impl IrBuilder {
    pub(super) fn build_text(&self, node: &NodeDecl) -> Result<TextData, PastelError> {
        ...
        for attr in &node.attrs {
            match attr.key.as_str() {
                "content" => {
                    t.content = p
                        .resolve_string(&attr.value)
                        .map_err(|e| e.with_span(attr.span))?
                }
                "size" => {
                    t.font_size = Some(
                        p.resolve_f64(&attr.value)
                            .map_err(|e| e.with_span(attr.span))?,
                    )
                }
                "weight" => {
                    t.font_weight = Some(
                        p.resolve_font_weight(&attr.value)
                            .map_err(|e| e.with_span(attr.span))?,
                    )
                }
                "font" => {
                    t.font_family = Some(
                        p.resolve_string(&attr.value)
                            .map_err(|e| e.with_span(attr.span))?,
                    )
                }
                "color" => {
                ...
                "align" => {
                    t.text_align = Some(
                        p.resolve_text_align(&attr.value)
                            .map_err(|e| e.with_span(attr.span))?,
                    )
                }
                "line-height" => {
                ...
                "width" => {
                    t.width = Some(
                        p.resolve_dimension(&attr.value)
                            .map_err(|e| e.with_span(attr.span))?,
                    )
                }
                "height" => {
                ...
                "wrap" => {
                    t.wrap = Some(
                        p.resolve_bool(&attr.value)
                            .map_err(|e| e.with_span(attr.span))?,
                    )
                }
                "letter-spacing" => {
                    t.letter_spacing = Some(
                        p.resolve_f64(&attr.value)
                            .map_err(|e| e.with_span(attr.span))?,
                    )
                }
                "text-decoration" => {
                    t.text_decoration = Some(
                        p.resolve_text_decoration(&attr.value)
                            .map_err(|e| e.with_span(attr.span))?,
                    )
                }
                "text-transform" => {
                    t.text_transform = Some(
                        p.resolve_text_transform(&attr.value)
                            .map_err(|e| e.with_span(attr.span))?,
                    )
                }
                _ => {}
    ...
    pub(super) fn build_image(&self, node: &NodeDecl) -> Result<ImageData, PastelError> {
        ...
        let mut img = ImageData {
            ...
            asset: node.name.clone().unwrap_or_default(),
            width: None,
            height: None,
            corner_radius: None,
            shadow: None,
        ...
        for attr in &node.attrs {
            match attr.key.as_str() {
                ...
                "radius" => {
                    img.corner_radius = Some(
                        p.resolve_corners(&attr.value)
                            .map_err(|e| e.with_span(attr.span))?,
                    )
                }
                "shadow" => {
                    img.shadow = Some(
                        p.resolve_shadow(&attr.value)
                            .map_err(|e| e.with_span(attr.span))?,
                    )
                }
                "opacity" => {
                ...
                "fit" => {
                    img.fit = Some(
                        p.resolve_image_fit(&attr.value)
                            .map_err(|e| e.with_span(attr.span))?,
                    )
                }
                _ => {}
    ...
    pub(super) fn build_shape(&self, node: &NodeDecl) -> Result<ShapeData, PastelError> {
        ...
        for attr in &node.attrs {
            match attr.key.as_str() {
                    ...
                    s.shape_type = match v.as_str() {
                        ...
                        _ => {
                            return Err(PastelError::new(
                                ...
                                format!("unknown shape type '{v}'"),
                            )
                            .with_span(attr.span)
                            .with_hint("expected: rectangle, ellipse, line, path"))
                        }
                    };
                }
                "path" => {
                ...
                "fill" => {
                    s.visual.fill = Some(
                        p.resolve_fill(&attr.value)
                            .map_err(|e| e.with_span(attr.span))?,
                    )
                }
                "stroke" => {
                    s.visual.stroke = Some(
                        p.resolve_stroke(&attr.value)
                            .map_err(|e| e.with_span(attr.span))?,
                    )
                }
                "stroke-dash" => {
                    let dash = p
                        .resolve_stroke_dash(&attr.value)
                        .map_err(|e| e.with_span(attr.span))?;
                    if let Some(ref mut st) = s.visual.stroke {
                        st.dash = Some(dash);
                    }
                }
                "radius" => {
                    s.visual.corner_radius = Some(
                        p.resolve_corners(&attr.value)
                            .map_err(|e| e.with_span(attr.span))?,
                    )
                }
                "shadow" => {
                    s.visual.shadow = Some(
                        p.resolve_shadow(&attr.value)
                            .map_err(|e| e.with_span(attr.span))?,
                    )
                }
                "inner-shadow" => {
                    s.visual.inner_shadow = Some(
                        p.resolve_shadow(&attr.value)
                            .map_err(|e| e.with_span(attr.span))?,
                    )
                }
                "opacity" => {
                    s.visual.opacity = Some(
                        p.resolve_f64(&attr.value)
                            .map_err(|e| e.with_span(attr.span))?,
                    )
                }
                ...
                "blend" => {
                    s.visual.blend = Some(
                        p.resolve_blend_mode(&attr.value)
                            .map_err(|e| e.with_span(attr.span))?,
                    )
                }
                "rotation" => {
                ...
                "position" => {
                    pos_mode = Some(
                        p.resolve_position_mode(&attr.value)
                            .map_err(|e| e.with_span(attr.span))?,
                    )
                }
                "top" => {
