...
use super::svg_effects::{
    rotation_attr, blend_attr, dash_attr, blur_filter,
    fill_str, write_shadow_def, shadow_filter_attr,
};
use super::svg_text::{render_text, render_image};
...
fn render_frame(
    ...
    let fill_a = f.visual.fill.as_ref()
        .map(|fill| fill_str(fill, &node.id, defs)).unwrap_or("none".into());
    let stroke_a = f.visual.stroke.as_ref()
        .map(|s| format!(r#" stroke="{}" stroke-width="{}""#, s.color.to_hex(), s.width))
        .unwrap_or_default();
    let opacity_a = f.visual.opacity.filter(|&o| o < 1.0)
        .map(|o| format!(r#" opacity="{}""#, o)).unwrap_or_default();
    ...
    if let Some(shadow) = &f.visual.shadow { write_shadow_def(defs, &node.id, shadow); }

    if has_kids {
        out.push_str(&format!("{indent}<g{opacity_a}{filter_a}{rot_a}{blend_a}>\n"));
        out.push_str(&format!(
...
fn render_shape(
    ...
    node: &IrNode, out: &mut String, defs: &mut String, indent: &str,
) {
    let fill_a = s.visual.fill.as_ref()
        .map(|fill| fill_str(fill, &node.id, defs)).unwrap_or("none".into());
    let stroke_a = s.visual.stroke.as_ref()
        .map(|st| format!(r#" stroke="{}" stroke-width="{}""#, st.color.to_hex(), st.width))
        .unwrap_or_default();
