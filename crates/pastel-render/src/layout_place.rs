...
use super::layout::{is_absolute, measure, pad, resolve_main, LayoutTree, Rect};
...
fn place_grid(
    flow: &[&IrNode],
    layout: &pastel_lang::ir::style::Layout,
    ix: f32,
    iy: f32,
    iw: f32,
    ih: f32,
    ...
    let sizes: Vec<super::layout::Size> = flow.iter().map(|ch| measure(ch, col_w, ih, c)).collect();
...
fn place_flex(
    flow: &[&IrNode],
    layout: Option<&pastel_lang::ir::style::Layout>,
    ix: f32,
    iy: f32,
    iw: f32,
    ih: f32,
    ...
    let mut sizes: Vec<super::layout::Size> =
        flow.iter().map(|ch| measure(ch, iw, ih, c)).collect();
    ...
    let fixed: f32 = sizes
        .iter()
        .map(|s| {
            if (is_h && !s.w_fill) || (!is_h && !s.h_fill) {
                if is_h {
                    s.w
                } else {
                    s.h
    ...
    let fill_n = sizes
        .iter()
        .filter(|s| if is_h { s.w_fill } else { s.h_fill })
        .count();
...
fn place_stack(
    ...
    for child in flow {
        ...
        tree.rects
            .insert(child.id.clone(), Rect { x: nx, y: ny, w, h });
...
fn place_absolute(
    ...
    let x = if let Some(left) = pos.left {
        px + left as f32
    } else if let Some(right) = pos.right {
        px + pw - w - right as f32
    } else {
        px
    };

    let y = if let Some(top) = pos.top {
        py + top as f32
    } else if let Some(bottom) = pos.bottom {
        py + ph - h - bottom as f32
    } else {
        py
    };
