...
use crate::painter_effects::{
    apply_blend_mode, apply_blur_filter, apply_dash_effect, apply_rotation, paint_inner_shadow,
};
use crate::painter_leaf::{
    make_gradient_shader, make_radial_gradient_shader, paint_image, paint_shadow,
};
...
fn paint_frame(
    ...
    if let Some(fill) = &f.visual.fill {
        ...
        match fill {
            Fill::Solid { color } => {
                paint.set_color4f(color_to_skia(color), None);
            }
            ...
            Fill::Transparent => {
                for child in &node.children {
                    paint_node(canvas, child, layout, images);
                }
                if has_blend {
                    canvas.restore();
                }
...
fn paint_shape(canvas: &Canvas, s: &pastel_lang::ir::node::ShapeData, rect: crate::layout::Rect) {
    ...
    let svg_path = s.path.as_ref().and_then(|d| {
        skia_safe::utils::parse_path::from_svg(d).map(|p| {
            ...
            let view_w = s
                ...
                .as_ref()
                .and_then(|d| match d {
                    pastel_lang::ir::style::Dimension::Fixed(n) => Some(*n as f32),
                    _ => None,
                })
